...
interface UpdateTechnologyDialogProps {
  technology: Technology;
  onUpdate: () => void;
  children: React.ReactNode;
}

export function UpdateTechnologyDialog({ technology, onUpdate, children }: UpdateTechnologyDialogProps) {
  const [open, setOpen] = useState(false);
  const [name, setName] = useState(technology.name);
  const [color, setColor] = useState(technology.color);
  ...
  useEffect(() => {
    if (open) {
      ...
      const [main, shade] = technology.color.split('-');
      setMainColor(main ?? '');
      setColorShade(shade ?? '');
    }
  }, [open, technology]);
  ...
  const validateForm = () => {
    ...
    if (!colorShade && mainColor !== "black") newErrors.color = 'Shade is required';
    ...
    if (validateForm()) {
      try {
        await updateTechnologyMutation.mutateAsync({
          id: technology.id,
          name,
          color: mainColor === "black" ? "black" : `${mainColor}-${colorShade}`,
          icon,
        });
        toast({
          title: "Technology updated successfully",
          description: "Your technology has been updated.",
        });
        ...
        toast({
          ...
          description: "There was a problem updating your technology. Please try again.",
          variant: "destructive",
        });
      }
    }
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid gap-4 py-4">
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              />
              {errors.name && <p className="col-span-3 col-start-2 text-red-500 text-sm">{errors.name}</p>}
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Select value={mainColor} onValueChange={setMainColor}>
                <SelectTrigger className={`col-span-3 ${errors.color ? 'border-red-500' : ''}`}>
                  <SelectValue placeholder="Select a color" />
                </SelectTrigger>
                <SelectContent>
                  {tailwindColors.map((colorOption) => (
                    <SelectItem key={colorOption.name} value={colorOption.name.toLowerCase()}>
                      <div className="flex items-center">
                        <div className={`mr-2 h-4 w-4 rounded-full ${colorOption.variations[1]}`}></div>
                        {colorOption.name}
                      </div>
                    </SelectItem>
                  ))}
              ...
              {errors.color && <p className="col-span-3 col-start-2 text-red-500 text-sm">{errors.color}</p>}
            </div>
                  ...
                  <SelectContent>
                    {tailwindColors
                      ...
                      ?.variations.map((variation) => (
                        <SelectItem key={variation} value={variation.split("-")[2] ?? ""}>
                          <div className="flex items-center">
                            <div className={`mr-2 h-4 w-4 rounded-full ${variation}`}></div>
                            {variation.split("-")[2]}
                          </div>
                        </SelectItem>
                      ))}
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Select value={icon} onValueChange={setIcon}>
                <SelectTrigger className={`col-span-3 ${errors.icon ? 'border-red-500' : ''}`}>
              ...
              {errors.icon && <p className="col-span-3 col-start-2 text-red-500 text-sm">{errors.icon}</p>}
            </div>
          </div>
